    ...
    return (
        <Case className='my-32 lg:flex flex-1' ref={inputEl}>
            <div className={`lg:w-1/2 ${textRight ? 'order-2' : ''} opacity-0 fadeIn--from-${textRight ? 'right': 'left'}`}>
                <div className='text-2xl muli leading-relaxed xl:leading-none flex justify-between'>{title} {github && <a href={github} rel='noreferrer' target='_blank' className='hover:no-underline flex items-center'><Github /></a>}</div>
                ...
                {link &&
                    <div className='my-8'>
                        <a href={link} rel='noreferrer' target='_blank' className='text-sm text-orange-600 border-bottom-hover'>ZUR WEBSITE <span className='text-lg'>›</span></a>
                    </div>
                }
            </div>
